...
import scheduling.FreeBlocksSchedulingDay;
...
public class ConflictsManager {
	...
	public void initializeCourses(List<LecturePOJO> lecturesList) {
		...
		List<String> courses = new ArrayList<String>(new HashSet<String>(lecturesList.stream().map(lecture -> lecture.getCurrentCourse()).collect(Collectors.toList())));
		
		
		List<String> semesters = new ArrayList<String>(new HashSet<String>(lecturesList.stream().map(lecture -> lecture.getCurrentSemester()).collect(Collectors.toList())));
		
		 
		this.scheduledCourseSemester =  lecturesList.stream().filter(lecture ->  
														 lecture.getCurrentCourse().equals(courses.get(0)) && lecture.getCurrentSemester().equals(semesters.get(0)) )
															    .collect(Collectors.toList())
															    	.stream()
		...
		try {
			...
			this.paralledCourseFirstSemester =  lecturesList.stream().filter(lecture ->  
																	 lecture.getCurrentCourse().equals(courses.get(1)) && lecture.getCurrentSemester().equals(semesters.get(0)) )
																		    .collect(Collectors.toList())
																		    	.stream()
																		    	.filter(lecture -> lecture.getisPractice() == false)
			...
			List<ScheduleLectureTimingPOJO> scheduledParalledCourseFirstSemester =  lecturesList.stream().filter(lecture ->  
             ...
             this.paralledCourseSecondSemester =  lecturesList.stream().filter(lecture ->  
																	 lecture.getCurrentCourse().equals(courses.get(1)) && lecture.getCurrentSemester().equals(semesters.get(1)) )
																		    .collect(Collectors.toList())
																		    	.stream()
																		    	.filter(lecture -> lecture.getisPractice() == false)
																		    	.map(lecture -> new ScheduleLectureTimingPOJO(lecture.getLectureName(), 
